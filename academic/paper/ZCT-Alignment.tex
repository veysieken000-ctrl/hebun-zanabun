...
\section{Connection to Large Language Model Alignment}

Modern large language models (LLMs) are typically aligned using
a combination of supervised fine-tuning and reinforcement learning
from human feedback (RLHF).

In these systems, a learned reward model $R_\theta$ acts as a proxy
for human preference. However, proxy-reward divergence may occur
under distribution shift or optimization pressure.

Within our framework, we interpret:

\begin{itemize}
\item $H(s,u)$ as the latent human value objective,
\item $E(s,u)$ as the learned reward model proxy,
\item $\lambda$ as the effective alignment weight
      controlling the influence of $H$ relative to $E$.
\end{itemize}

Under strong optimization pressure toward proxy maximization,
$\lambda$ may effectively decrease,
leading to increased drift and misalignment risk.

This interpretation reframes reward-model overoptimization
as a gradual alignment-collapse process.

\subsection{Reward Model Overoptimization}

In reinforcement learning from human feedback,
policies are optimized to maximize a learned reward model.

If the proxy reward model diverges from true human values,
increasing optimization strength can produce specification gaming.

In our formulation, such overoptimization corresponds to:

\[
\lambda \downarrow \quad \Rightarrow \quad Drift \uparrow
\]

Thus, alignment failure may emerge not from incorrect reward design,
but from dynamic reweighting under optimization pressure.
...
Refine abstract to include LLM alignment connection
...
\begin{abstract}
We propose a minimal alignment control model based on an
upper-measure attention parameter $\lambda \in [0,1]$.
Drift is defined as $1-\lambda$ and serves as a continuous
measure of alignment degradation.

We connect this framework to reward hacking,
distribution shift, and reward model overoptimization
in large language models.
A toy distribution-shift experiment illustrates
how proxy dominance increases alignment fragility.
\end{abstract}
...
This work presents a minimal conceptual and experimental model.
Several limitations should be acknowledged.

\subsection{Toy Environment}

The experimental validation relies on a simplified toy MDP.
While useful for illustrating dynamics,
it does not capture the complexity of modern large-scale models.

Future work should evaluate the framework in
more realistic reinforcement learning and language model settings.

\subsection{Fixed Upper-Value Assumption}

We assume the upper-value function $H$ is stable and well-defined.
In practice, value representations may themselves be uncertain
or partially observed.

Extending the framework to stochastic or learned $H$
remains an open problem.

\subsection{Single-Parameter Control}

The current model reduces alignment to a single scalar parameter $\lambda$.
Real systems may require multi-dimensional control structures
or hierarchical weighting schemes.

Future work may generalize the model to
vector-valued alignment parameters.

\subsection{Theoretical Guarantees}

Although we propose a collapse dynamic,
formal stability proofs and regret bounds are not yet derived.

Providing theoretical guarantees for bounded drift
